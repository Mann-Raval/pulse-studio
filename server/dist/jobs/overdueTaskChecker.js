...
exports.startOverdueTaskChecker = startOverdueTaskChecker;
...
async function checkOverdueTasks() {
    ...
    try {
        ...
        console.log(`[${timestamp}] ⚠️ [Cron:OverdueTaskChecker] Found ${overdueTasks.length} newly overdue tasks. Updating...`);
        let flaggedCount = 0;
        for (const task of overdueTasks) {
            try {
                ...
                if (targetUserId) {
                    ...
                    (0, index_js_1.broadcastNotification)(targetUserId, notif);
                }
                // Broadcast live WebSocket event
                (0, index_js_1.broadcastTaskOverdue)({
                    taskId: updated.id,
        ...
        console.log(`[${timestamp}] ✅ [Cron:OverdueTaskChecker] Successfully flagged ${flaggedCount} tasks as overdue.`);
        return flaggedCount;
    }
    catch (error) {
...
function startOverdueTaskChecker() {
    ...
    const scheduledJob = node_cron_1.default.schedule('*/5 * * * *', async () => {
